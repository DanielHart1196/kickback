...
export async function POST({ params, request }) {
  ...
  const response = await fetch(`${zeptoBaseUrl}/payto/agreements/${agreementUid}/cancellation`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    return json({ ok: false, error: result ?? 'cancel_failed' }, { status: response.status });
  }
