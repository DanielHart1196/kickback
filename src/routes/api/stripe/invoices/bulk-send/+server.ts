...
export async function POST({ request }) {
  try {
    ...
    for (const venue of venues ?? []) {
      ...
      if (!stripeCustomerId) {
        results.push({ venue_id: venue.id, ok: false, error: 'missing_stripe_customer' });
        continue;
      }
      ...
      try {
        ...
        results.push({
          ...
          subtotal,
          total: totalWithGst,
          amount_due: Number(finalized?.amount_due ?? invoice?.amount_due ?? 0) / 100,
    ...
    return json({ ok: false, error: error instanceof Error ? error.message : 'bulk_send_failed' }, { status: 500 });
  }
}
