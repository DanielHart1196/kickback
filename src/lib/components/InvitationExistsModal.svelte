...
<div class="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-6" transition:fade>
  <button
    ...
    aria-label="Close invitation warning"
    on:click={onDismiss}
  ></button>
  <div in:fly={{ y: 20, duration: 400 }} class="relative bg-zinc-900 border border-zinc-800 p-8 rounded-[2rem] w-full max-w-sm text-center shadow-2xl">
    ...
    <h2 class="text-2xl font-black text-white mb-3 italic uppercase tracking-tighter">Invitation Exists</h2>

    <p class="text-zinc-400 text-sm leading-relaxed mb-8">
      You already have an active or expired invitation for <span class="text-white font-bold">{venue || 'this venue'}</span>.
    </p>
