<script lang="ts">
  ...
  export let mode: 'retry' | 'nomatch' = 'retry';
...
<div class="fixed inset-0 bg-black/80 backdrop-blur-md z-[200] flex items-center justify-center p-6" transition:fade>
  <button
    ...
    aria-label="Close payment verification warning"
    on:click={onDismiss}
  ></button>
  <div in:fly={{ y: 20, duration: 400 }} class="relative bg-zinc-900 border border-zinc-800 p-8 rounded-[2rem] w-full max-w-sm text-center shadow-2xl">
    ...
    <h2 class="text-2xl font-black text-white mb-3 italic uppercase tracking-tighter">
      Unable to verify payment
    </h2>

    {#if mode === 'nomatch'}
      <div class="text-zinc-400 text-sm leading-relaxed mb-8 space-y-3 text-left">
        <p>- check purchase time is within 5 minutes</p>
        <p>- for Google Pay/Apple Pay, use the last 4 digits shown in your wallet app</p>
      </div>
    {:else}
